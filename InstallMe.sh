...
then
    which ruby > /dev/null;[ $? -eq 1 ] && sudo apt-get install ruby|| sleep 1
    which figlet > /dev/null;[ $? -eq 1 ] && sudo apt-get install figlet|| sleep 1
    which lolcat > /dev/null;[ $? -eq 1 ] && sudo gem install lolcat|| sleep 1
	...
	then 
        which ruby > /dev/null;[ $? -eq 1 ] && pkg install ruby -y || sleep 1
        which figlet > /dev/null;[ $? -eq 1 ] && pkg install figlet -y || sleep 1
        which lolcat > /dev/null;[ $? -eq 1 ] && gem install lolcat || sleep 1               
		which php > /dev/null
		if [ $? = 1 ];
		then
			pkg install php -y 
		...
		echo -e "\e[31m plase install jq unzip ruby php curl]"
	fi
fi
